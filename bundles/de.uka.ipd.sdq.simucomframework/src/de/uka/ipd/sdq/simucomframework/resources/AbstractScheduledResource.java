 ...
 */
public abstract class AbstractScheduledResource extends SimuComEntity
		implements IActiveResourceStateSensor, IResourceDemandModifiable {
	...
	/**
	 * Subclasses are responsible for creating the {@link IActiveResource} to use
	 * internally. Is called in the constructor.
	 * 
	 ...
	 * simulation. Allows passing of additional parameters.
	 * 
	 * @param thread            The thread or job requesting the processing of its
	 *                          demand
	 * @param resourceServiceID The id of the resource service to be called for
	 *                          resource consumption
	 * @param parameterMap      Additional parameters which can be used by the
	 *                          resource. Map can be empty.
	 * @param abstractDemand    The resource demand the client wishes to be
	 *                          processed by the resource
	 */
	public void consumeResource(SimuComSimProcess thread, int resourceServiceID, Map<String, Serializable> parameterMap,
		...
		// for every demand modifying behavior 
	 ...
	 * 
	 * @param demand The demand issued to this resource in units understood by the
	 *               resource
	...
	/**
	 * Called by the framework to inform the resource that the simulation has been
	 * stopped. Fires a {@link IStateListener#stateChanged()} event.
	 */
	 ...
	 * 
	 * @param isAvailable the target state to set
	 */
	...
	/**
	 * Returns the id of the resource type (e.g. "_oro4gG3fEdy4YaaT-RYrLQ" for the
	 * "CPU" resource type).
	 * <p>
	 * Be warned that this method's name may be confusing since it interprets the
	 * term "resource type id" different to the rest of SimuCom! In SimuCom, the
	 * variable name "typeID" is mostly used to refer to the name (!) of a resource
	 * type, i.e. "CPU", not "_oro4g...".
	 * <p>
	 * TODO: the {@code typeID} variables should be renamed to {@code typeName} or
	 * similar!
	 */
